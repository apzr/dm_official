 ...
 */
public interface CounselRepository extends PagingAndSortingRepository<Counsel, Integer> {

	@Query(value = "SELECT id, name, tel, mail, detail, create_time "
			+ " FROM t_counsel c "
			+ " WHERE id = :id", nativeQuery = true)
	public Counsel getAbout(@Param("id") Integer id);

	
	@Query(value = "SELECT id, name, tel, mail, detail, create_time "
			+ " FROM t_counsel c "
			+ " ORDER BY create_time LIMIT :limit ", nativeQuery = true)
	public List<Counsel> top(@Param("limit") Integer limit);
