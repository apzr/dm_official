 ...
 */
public interface ClassicRepository extends PagingAndSortingRepository<Classic, Integer> {

	@Query(value = "SELECT id,title,detail,release_time,create_time "
			+ " FROM t_classic c "
			+ " WHERE id = :id", nativeQuery = true)
	public Classic getClassic(@Param("id") Integer id);

	
	@Query(value = "SELECT id,title,detail,release_time,create_time "
			+ " FROM t_classic c "
			+ " ORDER BY create_time LIMIT :limit ", nativeQuery = true)
	public List<Classic> top(@Param("limit") Integer limit);
